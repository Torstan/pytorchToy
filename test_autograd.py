...
# 15a: save_for_backward works normally (no in-place modification)
x = Variable(torch.FloatTensor([3.0]), requires_grad=True)
y = Square()(x)
y.backward()
check("save_for_backward works (no in-place)", float_eq(x.grad.data[0], 6.0))

# 15b: detect in-place modification between forward and backward
x = Variable(torch.FloatTensor([3.0]), requires_grad=True)
y = Square()(x)
x.data[0] = 999.0  # in-place modification after forward, before backward
inplace_detected = False
try:
    y.backward()
except RuntimeError as e:
    inplace_detected = "inplace operation" in str(e)
check("in-place modification detected", inplace_detected)
