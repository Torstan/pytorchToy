...
assert compiled["count"] == 2, f"expected two compiled regions, got {compiled['count']}"

x_neg = torch.tensor(-1.0)
torch.testing.assert_close(opt(x_neg), demo(x_neg))
assert compiled["count"] == 2, "expected graph-break resume plan to be reused on cache hit"

torch.testing.assert_close(opt(x_pos), demo(x_pos))
assert compiled["count"] == 2

print("dynamo_graph_break_resume: ok")
